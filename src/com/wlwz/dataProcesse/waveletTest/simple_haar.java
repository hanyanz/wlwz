...
<p>
   This object calcalculates the "ordered fast Haar waveletTest
   transform".  The algorithm used is the a simple Haar waveletTest
   algorithm that does <u>not</u> calculate the waveletTest transform
   in-place.  The function works on Java double values.
<p> 
   The wavelet_calc function is passed an <b>array</b> of doubles from
   which it calculates the Haar waveletTest transform.  The transform is
   ...
   The number of data points in the data used to calculate the waveletTest
   must be a power of two.
<p>
   The Haar waveletTest transform is based on calculating the Haar step
   function and the Haar waveletTest from two adjacent values.  For
   an array of values S0, S1, S2 .. Sn, the step function and
   waveletTest are calculated as follows for two adjacent points,
   S0 and S1:
 ...
 *      
<p>
   Calculate the Haar waveletTest transform
   (the ordered fast Haar waveletTest tranform).
