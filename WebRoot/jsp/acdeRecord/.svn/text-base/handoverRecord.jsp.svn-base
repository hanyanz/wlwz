...
<script language="JavaScript" type="text/JavaScript">
   function add(){
       window.open('jsp/acdeRecord/handoverRecordAdd.jsp','新增',
   ...
   function check(str){
	...
	<s:form action="handoverrecordAction.action" theme="simple">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
       <tr>
         ...
         <td width="200px" ><input name="deviceNumber" type="text"/><s:submit value="查询" method ="getHandoverrecordList"/></td>
       </tr>
    </table>
	<table>
	 <thead>
	   ...
	   <td style="background-color: #d3eaef;">选择</td>
	    ...
	    <td width="200" height="20" bgcolor="#FFFFFF" class="STYLE19" style="text-align: center;background-color: #d3eaef;">转出单位</td>
	    <td width="100" height="20" bgcolor="#FFFFFF" class="STYLE19" style="text-align: center;background-color: #d3eaef;">接收单位</td>
	   </tr>
	  </thead>
	  <tbody  id="tb">
	  	...
	  	<c:forEach items="${queryList}" var="temp" varStatus="i">
	  		<tr onMouseOver="this.style.background='#d3eaef'" onMouseOut="this.style.background='#ffffff'" onclick="box1()">
	  			<td><input type="checkbox" class="checkbox" value="${temp.handoverRecordId}" name="handoverRecordId"/></td>
				 <td align="center">${i.count}</td>
				 <td align="center">${temp.device.deviceNumber}</td>
				 <td align="center">${temp.transferUnit }</td>
				 <td align="center">${temp.acceptUint }</td>
			</tr>
	 	</c:forEach>
     </tbody>
	   <tr>
